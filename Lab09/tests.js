...
describe('1d: Only wizards shall pass!', () => {
  ...
  if(movieCharacters.isAWizard== true) {
  it('#Should return character if a wizard', () => {
    expect(onlyWizards(character.isAWizard)).to.equal('true')
  }) 
} else {
  movieCharacters.isAWizard== false
  it('#Should not return character if not a wizard', () => {
    expect(movieCharacters.isAWizard).to.equal('false')
  }) 
}
})
})